...
- **Tide Quoted-stage cadence is 16 stage_entered automations, all install DISABLED.** `migrations/2026-05-07-tide-quoted-variants.sql` seeds Move-Out (4 rows: Day 1 call, Day 2 SMS, Day 4 SMS+call, Day 6 SMS final), Deep Clean (6 rows: Day 1 call, Day 2 SMS, Day 4 SMS+email, Day 6 SMS, Day 8 call, Day 10 SMS final), and Regular (6 rows: Day 1 call, Day 3 SMS, Day 5 SMS, Day 8 email, Day 11 call, Day 14 SMS final). Names follow the pattern `Tide · Quoted · {variant} · Day {N} {action_type}`. Idempotent — safe to re-run; uses `ON CONFLICT (name) DO UPDATE` so re-running refreshes trigger_config + actions but does NOT toggle is_enabled back to false (so once Dane has turned an automation ON, re-running won't shut it off). All SMS and email actions have `ai_personalize: true` so the message body adapts to the lead's call/SMS history. The literal `message` text in each row is the *fallback template* if AI personalize fails — keep them human-sounding so the fallback isn't jarring.
