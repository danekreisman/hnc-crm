...
*Last updated: May 5, 2026 — Audit tab redesigned as an animated "Automation Galaxy" — Canvas-rendered deep-space scene where each automation is a star system. Backend `/api/get-automations-galaxy.js` returns per-automation structure (trigger, step sequence, recent_runs hydrated with lead names, stats). Frontend has two views with cross-fade transitions: galaxy view (sunflower-spiral layout, parallax 3-layer starfield with drift, soft purple/blue nebulae, pulsing star cores with hue+size based on activity, comets orbiting each system representing recent leads, hover/click handling) and system detail view (constellation of trigger→delay→action→exit nodes connected with arc lines, comets traveling along the path, node flash + ripple on comet arrival, color-coded by step kind: blue=trigger, amber=delay, green/cyan/violet=action types, gray=exit). Live mode polls `/api/get-automations-galaxy` every 30s and spawns fresh comets with arrival flash for new runs; replay mode shows the last 10 runs per automation looping continuously. Mode preference persists in localStorage. Animation cleanup: `window._galaxyState` stashes rafId + pollTimer for cancellation on re-render to prevent memory leaks. ~730 lines of new client code, all-Canvas (no SVG/D3), no external deps. Hash+seeded-RNG ensures consistent system positions across re-renders. Performance tested for ~30 systems with ~10 comets each at 60fps. Removed: previous Sankey implementation (was replaced) and all `_AUDIT_LEGACY_CATALOG` static catalog code (no longer used — the galaxy view derives from `lead_automations` + `lead_automation_runs` only and intentionally does NOT show the legacy boolean toggles, since those don't have per-run history). If we later want to bring legacy toggles back into the audit, it'd be a separate "Cron schedule" view alongside the galaxy.*
