...
- **Waiver page (`agree.html`) loads BOTH policies and checklists from `settings` table** (added April 2026). Two endpoints power it: `/api/get-policies` returns `settings.policy_items`, and `/api/get-checklists` returns `settings.service_checklists`. Both have hardcoded fallback defaults in their `.js` files so the page never breaks if the DB rows aren't seeded. To edit waiver content live without code changes, edit the JSONB in those two `settings` rows directly in Supabase. Service checklists are organized as `{services:[{id,label,intro?,required?,sections:[{heading,items:[]}],notIncluded?,footnote?}],beforeArrival:[]}`. The `required` array (only present on move-out today) is NOT rendered inside the service card when `?svc=moveout` is in the URL — instead each `required` item is injected as its own policy checkbox so the customer must acknowledge it individually before the agree button enables. The titles for those injected policies are hardcoded in `MO_TITLES` array in agree.html — keep them in sync with the order of items in `services[2].required`. The `notIncluded` array renders as a dashed-border gray block at the bottom of the service card. The `id` field on each service maps to an emoji in `SERVICE_EMOJI` in agree.html — adding a new service requires updating that map. The `p5` policy ("Quote accuracy & on-arrival adjustment") is the contractual basis for raising prices on under-described moveouts; do not remove or weaken its language without thinking through legal implications. **Service-specific filtering: agree.html accepts `?svc=moveout` or `?svc=moveout,regular` URL params** — if present, only the listed services render. **Deep auto-pairs with Regular** in the filter (e.g., `?svc=deep` shows both Regular + Deep cards) because Deep's content references "everything in Regular plus...". All service cards render collapsed by default — customer taps to expand. The universal "Before we arrive" prep card is hidden when `moveout` is the only filtered service since move-out has its own (much stronger) Required-before-arrival policy block. To deploy on a new environment, run `supabase/add_service_checklists.sql` once in the SQL editor to upsert both rows.
