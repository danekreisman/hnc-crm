...
*Last updated: May 7, 2026 (later) — **Public booking review modal + Accept-and-schedule shipped.** Closes the remaining gap from the morning's public-booking work: Dane can now one-click accept a `review_public_booking` task and the system creates a real appointment. Same session also fixed three issues found during Dane's first live test: rush fee wasn't applied in the public flow, the request date wasn't visible in the in-app notification, and the new-quote address field had no Maps autocomplete. **Backend**: new endpoint `api/accept-public-booking.js` (Bearer-auth gated, mirrors `accept-call-lead.js`). On invocation: loads the task by `taskId`, verifies `type='review_public_booking'` + `status='open'`, reads `extracted_data`, then branches on `path`: (a) `new_quote` → calls the existing `book_lead_atomic` RPC with the lead_id, client_data, and appointment_data — same RPC the token-link auto-book uses, so client-create + appointment-insert + lead-close are atomic; (b) `existing_property` → direct INSERT into `appointments` keyed off the matched `client_id` from extracted_data, no lead row is touched. Either path then marks the task `completed` (non-fatal if it fails — appointment is the source of truth, an orphaned open task is just visual noise Dane can dismiss). Optional confirmation email gated by request body's `sendEmail` flag (default off so accepting during testing doesn't ping real customers). New schema `acceptPublicBooking` in `validate.js` (taskId required, optional date/time/notes/sendEmail overrides). **Frontend (index.html)**: (1) registered `review_public_booking` in the existing `typeLabel`/`typeColor` maps in `renderTaskCard` (label "New booking request", color `#3BB8E3` — brand blue). (2) Added "Review & accept" / "Decline" inline buttons to the task card for `review_public_booking` tasks, parallel to the existing review_call_lead pattern. (3) New overlay `#public-booking-review-overlay` between `call-lead-review-overlay` and `new-cleaner-overlay` in the markup. The overlay leads with a prominent date+time+price summary banner (so Dane sees the requested date *first* — that was the user-reported "I couldn't see what date the customer wanted" issue), then a red same-day/next-day/2-day rush banner if applicable, then sections for Customer (with tel:/sms:/mailto: links), Property & service, editable Schedule (date+time inputs prefilled from extracted_data — Dane can shift if he can't accommodate the original ask), read-only Pricing (base / GET / rush / total), Customer notes (only shown if non-empty), Admin notes textarea (gets appended to appointment notes), and a "Send confirmation email to customer" checkbox (default unchecked during testing per Dane's instruction to not message clients). (4) Three new JS functions next to the existing call-lead handlers: `openPublicBookingReview(taskId)` (populates the overlay from `_tasks.open[].extracted_data` and opens it), `confirmPublicBookingAccept()` (POSTs to `/api/accept-public-booking` with the user-supplied auth token and overrides, then closes the overlay, refreshes tasks + appointments, shows toast), `dismissPublicBooking(taskId)` (alias to `completeTaskById` — undo via the existing toast). **Frontend (book.html)**: (1) Rush fees re-enabled in the public flow — removed the `if (IS_PUBLIC) { ... return; }` short-circuit in `drawRushFee`. The fee is now passed through in the submit payload as `rushFee` and stored on the task's `extracted_data.rush_fee`; the accept endpoint reads it and folds it into the appointment's `total_price` and notes. (2) Google Places autocomplete on the new-quote address field — ported the IIFE pattern from `lead-form.html` (CSS classes prefixed `bk-pac-*` to avoid colliding with `lf-pac-*`). The address input gets wrapped in `<div class="bk-pac-wrap">` and `initBkAddressField()` is called from the end of `renderNewQuoteForm()` since the input is freshly innerHTML'd each time. Same `/api/places-autocomplete` endpoint backs both forms — no new infrastructure. **Backend (submit-public-booking.js)**: (1) `rushFee` added to `publicBookingSubmit` schema (max 500). (2) Reads `b.rushFee` and folds it into `displayTotal` (post-tax + rush) — for new_quote: `pretax + tax + rush`; for existing_property: `histPostTax + rush` (the historical price was the customer's normal post-tax total, so rush goes on top). (3) Stored as `extracted_data.rush_fee` so the accept handler can apply it to the appointment notes. (4) **In-app notification + push body now lead with the requested date and rush flag** — `prettyReq` formats as "Wed May 7 at 9:00 AM" and `rushTag` shows "⚡ SAME-DAY" when applicable. Title is "New booking request — [name]" (push) or "New booking request: [name]" (in-app), body is "[date+time] · [service] (frequency) · $[total]". This was the visibility fix Dane asked for. **Files**: `api/accept-public-booking.js` (new), `api/utils/validate.js` (added `acceptPublicBooking` schema, added `rushFee` to `publicBookingSubmit`), `api/submit-public-booking.js` (rush fee threading + prettified notification body), `book.html` (drawRushFee public-flow gate removed, payload includes rushFee, Places autocomplete wired), `index.html` (typeLabel/typeColor updated, action buttons block, overlay HTML, three JS functions). **Migrations**: none needed for this batch (the `review_public_booking` type was added in the morning's migration). **Test path (still safe — no client messages)**: submit any booking via book.html, find the task in the queue, click "Review & accept" — the overlay should show the requested date prominently. Edit date/time if needed, leave "Send confirmation email" unchecked, click "Accept & schedule". Verify a row appears in `appointments` with `status='scheduled'` (no cleaner_id) and `total_price` matching the displayed total including any rush fee. The task status should flip to `completed`. **Known issues / next-session items**: (1) The accept overlay doesn't yet have a cleaner-assignment dropdown — Dane assigns the cleaner from the calendar after acceptance. Adding an inline cleaner picker is a 30-min add when wanted. (2) The "Send confirmation email" checkbox defaults to OFF — flip the default to ON once Dane is past the testing phase. The endpoint already has the email-send code wired to `/api/send-email type='booking_confirmation'`. (3) The lookup endpoint still exposes `bookingToken` in its response (documented in the morning's entry) — unchanged this session.*
