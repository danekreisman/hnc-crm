...
*Last updated: May 5, 2026 — Automations Audit tab. New "Audit" tab added to the Automations view's tab nav (alongside Leads/Clients/Cleaners/Admin) — single read-only inventory of every automation across both systems. Pulls from `lead_automations` (custom stage_entered/segment automations with last_run_at + run stats) AND from `ai_booking_settings` (16 hand-curated legacy boolean toggles spanning inbound lead-capture, daily VA crons, hourly job-completion crons, customer-facing reminders, and internal task notifications). Each row shows: name, category, trigger description, on/off pill, last fire (custom only — legacy crons don't write per-key timestamps), and source location (table.column or file path). Summary header shows total/enabled/disabled counts. Implementation: new `renderAutomationsAudit()` in index.html with `_AUDIT_LEGACY_CATALOG` static array (~16 entries — add a row when a new legacy toggle ships), called by `_applyAutomationTab` when the Audit tab is selected. To extend: append to `_AUDIT_LEGACY_CATALOG`. Legacy timestamps could be tracked later by adding a per-key `last_fired_at` write to each cron's success path, but it's significant churn for marginal value — defer until a specific debug case needs it.*
