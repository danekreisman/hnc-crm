...
**HARD-LEARNED PATTERN — OPENPHONE SELF-TO-SELF:** Sending an SMS via `/api/send-sms` to the same number that's `QUO_NUMBER` (HNC business line, `+18084685356`) silently fails. OpenPhone returns a non-success status; the failure is logged but the message never reaches Dane. **Always send admin SMS to a different number than QUO_NUMBER.** Dane's personal `+18082697636` is the right destination. This bit twice in 2026-05-09 — first as the unfindable cause of Kai's missed notification on lead-book, then as a near-miss on the SMS floor I shipped earlier the same day for submit-public-booking before realizing it had the same bug. Both fixed in commit 27575e1.
