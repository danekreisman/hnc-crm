...
| Commit | What |
|---|---|
| `81d417a` (this session) | **Per-lead automation-exclusion toggle (do_not_contact).** UI was missing for the existing `leads.do_not_contact` column that the system crons already respected. Added: (1) new "Exclude from automations" pill on every lead profile above the action grid, optimistic UI with revert on DB error. (2) `doNotContact` field hydrated into `leadDB` on load so the pill renders correctly on profile open. (3) `/api/lead-followup-send` now returns 403 if `do_not_contact = true` — closes the gap where the toggle could be bypassed by hitting the AI follow-up button manually. The toggle directly controls behavior in `run-task-automations.js` (stage advance), `run-automations.js` (blacklist guard), `send-broadcast.js` — all already filtered on this column before this commit. Cancel-client flow at index.html line 8444 already sets this on the matched lead, so cancellations auto-blacklist. |
...
*Last updated: May 1, 2026 — Heavy session. AI follow-up button shipped (manual lead nurture with SMS/email/both, AI-generated, preview-and-edit). Several bug fixes: silent-revert on lead stage changes, manual-task 401, "Error: lead-overlay" false-positive on every save, scheduling duplication when editing recurring series, halt-series cross-contamination, Google Places dropdown stuck open. UX wins: CRM startup parallelized (pipeline renders 1-2s sooner), calendar defaults to current month + Today button, new-appt form starts blank, task undo + reopen + daily due-date SMS digest. Janitorial leads now have their own stage and segment so follow-up automations skip them. AI summary collapsed to "🚩 Things to know" bullets. Lead form launch checklist documented at top of Pending. Many fire-and-forget DB writes replaced with awaited+error-checked patterns to prevent future silent UI/DB drift.*