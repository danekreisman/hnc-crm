...
- **SMS-to-owner floor + silent-failure surfaces** (commit 7cab947, 2026-05-09). After the notification fix above didn't address WHY Dane saw nothing (push, email, AND the calendar banner all came up empty for Kai), added belt-and-suspenders: (a) `submit-public-booking.js` now sends an SMS to OWNER_PHONE via `/api/send-sms` as a 4th notification path — independent of the bell/push/email/banner/realtime stack. Even if every other layer breaks silently in the future, Dane's phone buzzes. Awaited inside `Promise.allSettled` like the others; failure → `logError`. (b) `loadTasks()` catch was `console.error` only — silent in production. Now routes through `_logFrontendError` so any future failure appears in the Recent Errors panel and the nav badge. This was the dangerous gap: if loadTasks silently failed, `_tasks.open` went empty, the banner showed nothing, Dane saw nothing. (c) `renderPublicBookingBanner` now logs when a `review_public_booking` task is in `_tasks.open` but lacks `extracted_data` — that case was being silently filtered out and the user had no signal a booking was pending.
