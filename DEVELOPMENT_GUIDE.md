...
| Commit | What |
|---|---|
| `a5c3c8c` (this session) | **AI summary timeout fix.** A long-time customer with deep history was timing out the structured Sonnet summary at the default 15s `TIMEOUTS.ANTHROPIC`. Bumped to **45s** for both `/api/ai-summary` (Sonnet path only — Haiku legacy callers still use 15s) and `run-task-automations.js::generateCallBrief`. If summaries start timing out again at this volume, the root cause to investigate first is OpenPhone history fetch latency, not the Anthropic call. |
