...
- **Quote detection treats `low` confidence as a rejection.** Both code paths require `confidence` ∈ {`'high'`, `'medium'`} AND a numeric `amount > 0` before creating a task. The prompt is intentionally conservative — false positives (a "ballpark" or deflected price) become spam tasks Dane has to dismiss, which trains him to ignore the channel.
