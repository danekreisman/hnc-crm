...
*Last updated: May 7, 2026 (evening) — **Notification click → review modal routing fix.** Bug found during Dane's first test of Accept: clicking the in-app notification opened the lead profile (with the generic "Book appointment" button) instead of the new public-booking review overlay. He thought the modal didn't have an Accept button at all — it was actually a different overlay opening entirely. Cause: the submit endpoint creates the notification with `event_type='lead_inquiry'` + `metadata.leadId`, and the existing `handleNotifClick` router has a `lead_inquiry + leadId` branch that calls `openLead(leadId)`. That branch ran first and routed to pipeline, never reaching anything that would open my new modal. **Fix has three parts**: (1) `submit-public-booking.js` now captures the new task's id from the `Prefer: return=representation` task-insert response (`taskRowId` parsed out of `taskRes.json()`) and includes it as `metadata.taskId` on the notification row. (2) `handleNotifClick` in `index.html` got a new branch placed BEFORE the existing `lead_inquiry → openLead` case: when `event_type='lead_inquiry'` AND `meta.source === 'public_booking'` AND `meta.taskId`, it routes to `/#tasks` and (after a 250ms paint delay) calls `openPublicBookingReview(meta.taskId)`. The existing lead-profile route still fires for non-booking lead_inquiry notifications, so other flows are unaffected. (3) `openPublicBookingReview` is now async and falls back to fetching the task row directly from Supabase if it's not in the in-memory `_tasks.open` cache — needed because clicking a notification typically lands the user on a view where loadTasks hasn't run yet, so the cache miss has to be handled gracefully or every notification click would show "Task not found." The select pulls the same columns loadTasks uses. The function also checks `task.status === 'open'` and shows "This booking has already been processed" if it isn't (handles the case where Dane clicks an old notification for a booking he's already accepted/declined). **Pattern codified**: when a notification needs to deep-link to something more specific than the generic event_type → view mapping, include a discriminator in metadata (`source: 'public_booking'`) AND the entity id needed to open the detail (`taskId`), and add the routing branch BEFORE the broader event_type handler. The handleNotifClick router uses first-match-wins. **No migrations.** **Test**: submit a booking via book.html (no `?bt=`). When the bell ding fires in the CRM, click it — should land on the tasks view with the public-booking review modal already open, showing the customer name, requested date prominently, rush banner if applicable, and Accept & schedule / Cancel buttons at the bottom. The tasks-queue inline "Review & accept" button still works exactly as before. **Still pending from earlier this session**: (1) Three-button UX (Accept / Edit / Deny) instead of current two-button (Accept & schedule / Cancel) — Dane wants this but we haven't agreed on what "Edit" means yet. Awaiting his clarification on whether it's (a) more inline-editable fields, (b) opens the full appointment-creation overlay pre-filled, or (c) saves changes to the request without accepting. (2) Cleaner assignment dropdown on the modal — still done from the calendar after acceptance. (3) "Send confirmation email" still defaults OFF for testing.*
