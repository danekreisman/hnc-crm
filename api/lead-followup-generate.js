...
export default async function handler(req, res) {
  ...
  try {
    ...
    const stageContext = (() => {
      ...
      if (stage === 'Long-Term Follow-Up' && hasPriceEvidence) return 'Cold lead — they got a quote (or prices were discussed by SMS) but went silent. Light, no-pressure check-in. If a price is on record or visible in SMS history, you may reference it. Don\'t guilt them. The aim is to leave the door open without being pushy.';
      if (stage === 'Long-Term Follow-Up' && !hasPriceEvidence) return 'Cold lead, but I do NOT have a record of a structured estimate being sent and no prices appear in SMS history. May have been a phone-only conversation. DO NOT claim to have sent an estimate. Open-ended re-engagement only: "wanted to circle back about your cleaning needs", "let me know if there\'s anything I can help you sort out". Reference details from the SMS/call history if any exist; otherwise keep it short and general.';
