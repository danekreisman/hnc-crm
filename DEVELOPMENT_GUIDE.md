...
*Last updated: May 5, 2026 — Audit tab upgraded from a static table to an animated Sankey flow diagram. New backend endpoint `/api/get-automations-flow.js` aggregates 90 days of `lead_stage_events` into nodes (lead sources → stages → automations) and links (counts per transition + per automation fire), with a configurable `?days=` window (7-180). Frontend lazy-loads d3@7 + d3-sankey@0.12 from jsdelivr on first Audit-tab open, renders SVG nodes/links with d3-sankey layout, and overlays a Canvas with animated particles flowing along each link path (sampled 80 points, rAF loop). Particles colored purple for automation fires, gray for lead flow; particle count + speed scale with link volume. Tooltip on hover shows in/out flow + automation fires per node, value per link. Time-range selector (30/60/90/180d) persists in localStorage. "has_enough_data" flag (≥30 transitions) triggers a soft "building flow data" message under the chart. Stats header shows total leads / transitions / automation fires for the window. Old static table view replaced — if you ever need it back, prior version is in commit history. Top 6 sources shown directly, rest bucketed as "Other sources" to keep the layout clean. Lazy-loading prevents the 70KB d3 cost until someone opens the tab.*
