...
| Commit | What |
|---|---|
| (this session, post-2-stage classifier) | **Lead profile autofill button.** New "✨ Auto-fill from history" button next to the Opportunity section header in the lead profile. Click → `/api/lead-autofill` pulls all SMS + call history for the lead's phone via `getOpenPhoneHistory` (the shared utility from `api/utils/openphone-history.js`), runs Claude Haiku extraction over the full conversation, returns suggested values + which lead fields are currently empty. UI shows a confirm dialog with the proposed fills; user accepts → fields update via `db.from('leads').update(patch)`. Default behavior: only fills EMPTY fields (won't overwrite manual entries). Solves the workflow gap where leads who came in via call/SMS had empty structured data and Dane was scrolling through call summaries / SMS threads to find info. Endpoint is auth-gated (Bearer token verified via Supabase auth). Lucky reuse: the other AI agent had already built `getOpenPhoneHistory` for use in `run-review-requests.js` / `ai-personalize.js` / `ai-summary.js` — fetches up to 200 SMS + 25 call summaries with formatted output ready for AI consumption. New file `api/lead-autofill.js` (~145 lines), modifications to `index.html` (button + handler + confirm flow). |
