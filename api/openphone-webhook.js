  ...
  */
  ...
  async function extractLeadFromSmsThread({ thread, currentMessage, senderPhone, fallback }) {
    ...
    const prompt = [
      ...
      '  - notes: ONLY operational details a CLEANER needs on-site to do the job. Before including anything, ask: "Would a cleaner pulling up to this property RIGHT NOW need to know this to do the job?" If no, leave it out.',
      '      INCLUDE: parking, gate codes, how to enter, pets, areas to focus on or skip, property quirks (steep driveway, no AC, shoes off), how to find the unit.',
      '      DO NOT INCLUDE (these have their own fields, or are not for the cleaner):',
      '        - "Studio unit", "3 bed 2 bath", "1200 sqft" → beds/baths/sqft fields',
      '        - "Deep clean", "weekly service" → service/frequency fields',
      '        - "Quoted $135", "wants $500/month plan", any pricing or discounts',
      '        - "Phone: +1...", emails, names, addresses → own fields',
      '        - "Prefers same cleaner", "wants female cleaner" → dispatch concern, not on-site',
      '        - "Interested in monthly service", "needs to discuss with partner" → sales status',
      '        - "Found us via Google", "Yelp lead" → not for cleaner',
      '        - Any summary of who they are or what they want',
      '      Return null when there are no on-site operational details — empty/null IS the correct answer for most leads.',
      '',
  ...
  async function classifyAndExtractCallLead({ transcript, summary, callerPhone, durationSeconds }) {
    ...
    const prompt = [
      ...
      '        - "Found us via Google", "Yelp lead" → not for cleaner',
      '        - Any summary of who they are or what they want',
      '      Empty string when there are no on-site operational details — empty IS the correct answer for most leads.',
