...
*Last updated: May 7, 2026 — **Public booking form shipped.** `book.html` now handles both the existing token-link flow (`?bt=<bookingToken>` → /api/lead-book auto-book, untouched) AND a new no-token public flow for the website / marketing links / broadcast emails. Public flow: lookup gate (email + phone, both required) → `/api/lookup-customer-bookings` → cards grid. Three card types render based on lookup match: (a) "Use my saved quote" appears for any open active lead with a quote (matched by email OR phone last-10) — clicking redirects to `/book.html?bt=<token>` so it goes through the existing auto-book flow with no behavior change for that path; (b) "Book your usual cleaning" appears for matched clients with a most-recent paid/completed appointment, showing their last total_price as the price-lock-in (the existing returning-customer pricing pattern); (c) "Get a new quote" always appears, branching into an inline form (service picker → service-specific pricing fields → optional frequency for Regular Cleaning → calls `/api/calculate-quote` server-side → synthesizes a LEAD-shaped object and renders the existing step 1/2/3 UI). New endpoint `/api/submit-public-booking` handles both `path='new_quote'` and `path='existing_property'` submissions: validates → recomputes price for new_quote (never trusts client-supplied price) → finds-or-creates lead (new_quote only — existing_property uses the matched client without creating a residual lead in the pipeline) → creates a `review_public_booking` task with ALL booking details in `extracted_data` (mirrors the `review_call_lead` pattern) → fires in-app notification + push fan-out + owner email + OpenPhone contact creation, all best-effort/non-blocking so a Resend/OpenPhone blip never fails the booking. NO customer-facing SMS or email is sent — the customer sees an in-page "Booking request received, we'll confirm within 24 hours" success message. Token-link path stays at `/api/lead-book` and continues to auto-book + auto-confirm exactly as before. Migration `2026-05-07-public-booking-review-task.sql` adds `review_public_booking` to the tasks_type_check constraint (required, otherwise the task insert silently fails the CHECK). Schemas added: `publicBookingLookup` (email+phone) and `publicBookingSubmit` (full body) in `validate.js`. **Files**: `api/lookup-customer-bookings.js` (new), `api/submit-public-booking.js` (new), `migrations/2026-05-07-public-booking-review-task.sql` (new — MUST be run manually in Supabase before the feature works), `book.html` (extended with public flow — existing token flow intentionally untouched), `api/utils/validate.js` (added 2 schemas). **Architectural notes**: (1) The `review_public_booking` task is the source of truth — no pending appointment row is created at submit time. Dane reviews the task and confirms, which is where the actual appointment should get created (next-session work — currently the task UI doesn't have a "confirm and schedule" handler for this task type). (2) Service-area gating is UI-only ("We service Oahu and Maui only" notice). Backend doesn't reject out-of-area submissions; Dane catches them at review. (3) Rush fees are suppressed in public flow (`drawRushFee` short-circuits when `IS_PUBLIC` is true) because pending review can't honor an instant same-day rate guarantee — Dane decides if a rush fee applies during review. (4) `priceTotal` for the existing_property path comes from the frontend (sourced from `lastAppt.totalPrice` in the lookup response) and is post-tax — `submit-public-booking.js` divides by `1 + TAX_RATE` to recover pre-tax for storage in `extracted_data.quote_total_pretax`. **Bug fixes shipped same session**: (a) the lead-update branch in `submit-public-booking.js` for the returning-lead-with-new-quote case now destructures `error` per the dev-guide silent-update gotcha; (b) `toE164()` is robust against 11-digit inputs (strips a leading '1' before re-prepending '+1', plus always slices to last 10) — the `'+1' + digits` pattern used elsewhere in the codebase produces malformed E.164 like `+118085551234` if a customer types `1-808-555-1234` or `18085551234`. **Known limitation**: the lookup endpoint exposes `bookingToken` in its response for matched leads. An attacker who knows a customer's email + phone could pull the booking token and auto-book on their behalf via the token-link flow (which bypasses pending review). Severity is moderate — requires knowing both email AND phone of an HNC customer with an open quote. Mitigation if it ever becomes a real problem: add an OTP step (SMS code) before showing cards, or remove the existing-quote card entirely (force everyone through pending review). **NOT done this session**: the in-app "Confirm & schedule" workflow on the `review_public_booking` task — Dane currently sees the task with all booking details in the description but has to manually create the appointment elsewhere. Next session priority is wiring an Approve handler that creates the appointment from `extracted_data` using the same `book_lead_atomic` pattern (or a sibling RPC) and assigns a cleaner. **Test path (no client messages — safe to run)**: open `https://hnc-crm.vercel.app/book.html` (no `?bt=`), submit any of the three paths. The customer never receives an SMS or email — only Dane gets the in-app task + push + owner email. To verify: check `tasks` table for a row with `type='review_public_booking'` after submitting; check `error_logs` for any `submit-public-booking:*` rows if something appears stuck.*
