# HNC CRM Ã¢ÂÂ Development Guide
...
Claude deploys directly Ã¢ÂÂ Dane is not in the deploy loop. When Dane asks for a fix: clone (or reuse) the repo in Claude's own environment, do surgical `str_replace` edits, sanity-check with `node --check`, commit, and push. Vercel auto-deploys from `main` in ~60s. Test against the live site, loop until it works.

Don't generate Claude Code prompts, don't output the patched file for Dane to copy, don't tell Dane to run git commands. The only acceptable fallback is if `git push` itself fails (auth, network) Ã¢ÂÂ in which case report the failure plainly and don't pretend the deploy happened.

### One problem, one fix, one deploy, one test Ã¢ÂÂ then loop.
...
5. **One problem per session.** Don't bundle multiple unrelated fixes Ã¢ÂÂ it increases the chance of regressions.
...
- Don't echo it back into chat unnecessarily. It's already in context once Ã¢ÂÂ don't repeat it.
...
If `git push` fails (auth, network, branch protection), report the exact error to Dane immediately. Do NOT silently fall back to outputting the patched file or asking Dane to push manually Ã¢ÂÂ that defeats the whole point of the new workflow. Dane's environment is no longer a dependency.
...
**Active integrations:**
- Supabase Ã¢ÂÂ core database (all data lives here)
- Stripe Ã¢ÂÂ invoicing and card charging (`/api/stripe-invoice.js`)
- OpenPhone/Quo Ã¢ÂÂ SMS sending and webhook receiver (`/api/send-sms.js`, `/api/openphone-webhook.js`)
- Resend Ã¢ÂÂ transactional email (`/api/send-email.js`)
- Anthropic Ã¢ÂÂ AI summaries (`/api/ai-summary.js`)
- Google Calendar Ã¢ÂÂ appointment sync (`gcal-sync.js`)
...
1. **Form HTML** Ã¢ÂÂ `#nl-name`, `#nl-beds`, `#nl-baths`, `#nl-condition`, `#nl-freq`, etc. (in the New Lead modal)
2. **`_buildNLQuote()`** Ã¢ÂÂ reads form values and computes `{ total, data: {...} }`. Lives near `nlServiceChange`. The `data` object MUST use these field names or the Quote subsection won't render: `service`, `subtotal`, `discount`, `discount_pct`, `total`. Other fields are fine but those five are required by `loadSuggestedQuote`.
3. **`saveNewLead()` in-memory cache** Ã¢ÂÂ `leadDB[id]={...}` block populates the in-memory object IMMEDIATELY so the card renders before the Supabase round-trip. Must include: `value`, `quoteTotal`, `quoteData`, `frequency`, `condition`, `beds`, `baths`. `var _nlQ = _buildNLQuote()` MUST be declared BEFORE this block so the values are available.
4. **`dbSaveLead(data)`** Ã¢ÂÂ tiny wrapper (~750 chars) that does `db.from('leads').insert([{...}])` with a HARDCODED field map. Silently drops any field not in its whitelist. As of commit 849f6a5 it includes: name, contact_name, phone, email, address, service, sqft, estimated_value (mapped from `data.value`), beds, baths, condition, quote_total, quote_data, source, stage, next_action, due_date, assigned_to, notes. **If you add a new column to the leads table you MUST add it here too.**
5. **`dbLoadLeads()` Ã¢ÂÂ leadDB mapping** Ã¢ÂÂ page-load mapping that turns Supabase rows into `leadDB[id]` objects. Lives near `async function dbLoadLeads`. As of commit 016341f, beds/baths/condition/frequency are read from actual columns (`l.beds`, etc.), with fallback to regex-parsing them out of the notes field for backwards-compat with old leads. Map fields with snake_case Ã¢ÂÂ camelCase: `l.quote_data Ã¢ÂÂ quoteData`, `l.quote_total Ã¢ÂÂ quoteTotal`, `l.estimated_value Ã¢ÂÂ value`.
6. **`openLead(id)` panel renderer** Ã¢ÂÂ sets the lead detail panel fields. Generic fields use a `['contact','phone','email',...].forEach` loop that maps `d[f] Ã¢ÂÂ #lead-${f}.textContent`. Special-case fields (frequency, bedbath, condition, value) have their own setter blocks. **Adding a new visible field requires both an HTML `<div class="info-row">` and a setter in openLead.**
...
```html
<div class="info-row"><span class="info-label">My Field</span><span class="info-val" id="lead-myfield">Ã¢ÂÂ</span></div>
```
...
if(myEl) myEl.textContent = d.myField || 'Ã¢ÂÂ';
```
...
- **Magic link** via `db.auth.signInWithOtp({ email })` Ã¢ÂÂ used by VAs (e.g., Leo). Email delivered through the Resend SMTP setup on the Supabase project.
- **Google OAuth** via `db.auth.signInWithOAuth({ provider: 'google' })` Ã¢ÂÂ primary for admins.

UI gating after sign-in is done client-side in `applyUserRole(email)`:
- Email in `ADMIN_EMAILS` Ã¢ÂÂ no class added, full UI.
- Otherwise Ã¢ÂÂ `hnc-va-user` class added to `<body>`, which hides admin-only sections via CSS rules at the top of `index.html`.
...
| `ADMIN_EMAILS`, `initAuth`, `applyUserRole`, `_setUserFooter`, `hncSendMagicLink`, `hncSignInWithGoogle`, `hncSignOut` | `index.html` ~lines 14575Ã¢ÂÂ14619 |

### Adding a new OAuth provider
1. Enable provider in Supabase Studio: **Authentication Ã¢ÂÂ Providers Ã¢ÂÂ [Provider]**.
...
4. In Supabase: **Authentication Ã¢ÂÂ URL Configuration Ã¢ÂÂ Redirect URLs**, ensure `https://hnc-crm.vercel.app` (and any other deploy domains) are allowlisted.
...
Clicking the provider's button surfaces an error in `#hnc-login-msg` like *"Unsupported provider: Provider is not enabled"*. The user stays on the login overlay. If you see this, the fix is on the Supabase config side Ã¢ÂÂ not in the frontend code.
...
### 1. Validation Ã¢ÂÂ `api/utils/validate.js`
...
- Never trust data from `req.body` without validating it first Ã¢ÂÂ this includes data generated by AI

### 2. Error Logging Ã¢ÂÂ `api/utils/error-logger.js`
...
- First argument is the source label Ã¢ÂÂ use the filename (e.g. `'lead-capture'`)
- Third argument is context Ã¢ÂÂ include any IDs or inputs that would help you debug
- `logError` never throws, so it's safe to call anywhere
- Check `error_logs` table in Supabase when something breaks

### 3. Timeouts Ã¢ÂÂ `api/utils/with-timeout.js`
...
### 4. Atomic DB Operations Ã¢ÂÂ `api/utils/webhook-idempotency.js` + Supabase RPC
...
1. Add the new type to the whitelist in `api/utils/validate.js` Ã¢ÂÂ `validateActions()` Ã¢ÂÂ `validTypes` array
...
- **Always validate AI output** before writing it to the database Ã¢ÂÂ use `validate()` just like you would for user input
- **Always set a prompt length limit** Ã¢ÂÂ add it to `SCHEMAS.aiSummary` or create a new schema
- **Never let AI write directly to the DB** Ã¢ÂÂ AI output should go through the same API endpoints as everything else, so validation runs on it
- **Log AI errors separately** Ã¢ÂÂ use `logError('ai-feature-name', err, { prompt: prompt.slice(0, 200) })`
- **Add timeouts** Ã¢ÂÂ use `TIMEOUTS.ANTHROPIC` (15s) for all Anthropic calls

---

## Supabase Rules

- **Never use `SUPABASE_ANON_KEY` for server-side writes** Ã¢ÂÂ use `SUPABASE_SERVICE_ROLE_KEY` in API functions
- **Keep RLS disabled** until going to production Ã¢ÂÂ when you're ready, enable it and lock down policies per table
...
- **Google Places autocomplete** was added and removed Ã¢ÂÂ it broke the address input. Don't reintroduce it without careful testing on the address fields first.
...
- **`run-automations.js`** is not yet using `logError` Ã¢ÂÂ errors only go to console. Add it when touching that file.
- **`lead-capture.js`** does not have an atomic transaction like `lead-book.js` does Ã¢ÂÂ if it fails midway you can end up with a lead but no email/SMS sent.
...
- **HTML script-tag balance after big edits.** If raw JS code starts appearing as text on the page, the script open/close count is unbalanced. Use `(html.match(/<script[^>]*>/g)||[]).length` and `(html.match(/<\/script>/g)||[]).length` to verify equal count. Premature `</body></html>` tags in the middle of the file are a common cause Ã¢ÂÂ search for multiple occurrences with `[...html.matchAll(/<\/html>/g)]`.
...
- **AI summary API accepts EITHER `{ prompt }` or `{ leadData }`** but not just `{ leadId }` Ã¢ÂÂ it has no Supabase fetch in its current form. Frontend builds the prompt and sends it.
- **In-memory leadDB after `saveNewLead`.** Even after Supabase save succeeds, the card renders from `leadDB[id]` BEFORE the next `dbLoadLeads()`. So the in-memory object built in `saveNewLead` must include quoteTotal, quoteData, frequency, condition, beds, baths Ã¢ÂÂ otherwise the card shows TBD until a hard refresh.
...
| Commit | What |
|---|---|
| (this session) | `DEVELOPMENT_GUIDE.md` Ã¢ÂÂ document activity log coverage, client profile modal, calendarÃ¢ÂÂclient link, browser-editor workflow, new gotchas |
| `53df3d6` | `index.html` Ã¢ÂÂ wire up client profile Stats (Lifetime, Total jobs, Avg, Monthly) from appointments |
| (in main) | `index.html` Ã¢ÂÂ Job History wired up + calendar appointment Client field clickable Ã¢ÂÂ opens client profile |
| `65dfffa` | `index.html` Ã¢ÂÂ add Automation log section to client profile modal + `loadClientActivityLog()` function |
| `62b5a21` | `api/tasks.js` Ã¢ÂÂ log VA-task email to activity_logs (direct Resend bypass case) |
| `41f30a0` | `api/send-sms.js` Ã¢ÂÂ log every successful SMS to activity_logs |
| `77e029f` | `api/send-email.js` Ã¢ÂÂ log every successful email send to activity_logs |
...
- Any caller hitting `/api/send-email` Ã¢ÂÂ logged with action `email_sent_${type}` (e.g. `email_sent_invoice`, `email_sent_reschedule`, `email_sent_thankyou`, `email_sent_booking_confirmation`, `email_sent_generic`). Metadata: `to`, `subject`, `type`, `clientName`, `resend_id`.
- Any caller hitting `/api/send-sms` Ã¢ÂÂ logged with action `sms_sent`. Metadata: `to` (E.164), `message_length`, `openphone_id`.
- All `run-*.js` cron handlers route through these endpoints, so they inherit logging.

### Manual coverage (one-off)
- `api/tasks.js` Ã¢ÂÂ VA-task email uses Resend directly (not via `/api/send-email`), so it has its own logActivity call. Action: `email_sent_va_task`.

### Intentionally excluded
- `api/send-broadcast.js` Ã¢ÂÂ uses Resend directly with custom logic. Per Dane's spec, broadcast sends are NOT logged to activity_logs.
...
### Schema reminder
`activity_logs` columns: `id, created_at, action, description, user_email, entity_type, entity_id, metadata (jsonb)`. The recipient is stored in `metadata.to` Ã¢ÂÂ that's the field the client profile filters on.
...
- `loadNotifPrefs(id)` Ã¢ÂÂ populates notification toggles (defined twice in source Ã¢ÂÂ see Known Gotchas)
- `loadClientActivityLog(id)` Ã¢ÂÂ fills `#cl-activity-log` with rows from activity_logs filtered by client's email/phone
- `loadClientStats(id)` Ã¢ÂÂ fills the stat tiles
- `loadClientHistory(id)` Ã¢ÂÂ fills `#cl-history` with past completed/paid appointments

### Stats calculation rules (do not break)
- "Done jobs" = appointments with `status IN ('completed', 'paid')`. Note that `paid` is by far the most common done state (~855 rows) vs `completed` (~135). **Do NOT filter only on `status='completed'`** Ã¢ÂÂ you'll miss ~85% of data.
...
### Calendar appointment Ã¢ÂÂ client profile link
...
2. If it's data-driven, add a loader function near the end of the script (just before the last `</script>` tag Ã¢ÂÂ use `text.lastIndexOf('</script>')` for a guaranteed-unique anchor).
3. Wire the loader call inside `openClient(id)` after `loadNotifPrefs(id);`.

---

## Browser-Editor Workflow (legacy fallback Ã¢ÂÂ rarely needed)
...
### The fetch Ã¢ÂÂ modify Ã¢ÂÂ clipboard Ã¢ÂÂ paste loop
...
4. Write modified text to clipboard: `await navigator.clipboard.writeText(modified)`. Requires document focus Ã¢ÂÂ if it throws "Document is not focused", click anywhere on the page first.
...
7. Click the green "Commit changesÃ¢ÂÂ¦" button (top right). Coordinates depend on scroll position: `y=149` if at top of page, `y=85` if scrolled. Take a screenshot first to be sure.
...
- **Triple-click hit editor instead of dialog.** If the commit dialog didn't actually open (because the click missed the button), the next triple-click selects code in the editor Ã¢ÂÂ and typing OVERWRITES that code. Recovery: click in editor, `cmd+z` multiple times to undo, then re-paste.
- **Content filter blocks JS output.** The Chrome MCP filter strips output containing certain patterns: full URLs (`api.resend.com`), base64 data, certain keys/secrets. Workarounds: (a) return char codes via `Array.from(s).map(c => c.charCodeAt(0))` Ã¢ÂÂ guaranteed safe; (b) replace bracket-like chars with placeholders before returning; (c) avoid printing matched text Ã¢ÂÂ just print indices/counts.

---

## Updates to Known Gotchas (additions for the section above)

- **`index.html` has duplicate `loadNotifPrefs` definitions** at lines ~14331 and ~16551. JS hoisting means the last one wins. When adding new helper functions referenced from `openClient`, inject them at the very end of the script (using `text.lastIndexOf('</script>')`) Ã¢ÂÂ this avoids picking the wrong duplicate as your insertion anchor.
...
- **Appointment status values are `paid` (~855), `completed` (~135), `cancelled` (~10)**. `paid` is by far the most common "done" state. Always filter `IN ('completed', 'paid')` when querying for done jobs Ã¢ÂÂ never only `status='completed'`.
...
- **Booking form's property dropdown loads via two triggers, not one.** `populatePropertySelector(name)` populates `#na-property-select` with the client's saved properties (from `clients.properties` JSON array) and auto-selects the first. It's called from (1) the autocomplete-suggestion click handler in `na-name`, AND (2) `na-name`'s `onblur`. If you ever wire a third path that fills the name programmatically (drag-and-drop, deep link, etc.), call `populatePropertySelector(name)` from there too Ã¢ÂÂ otherwise the user lands in "+ Create New Property" mode and beds/baths/sqft default in instead of carrying over from the saved property.
- **Properties are stored as a JSON array on `clients.properties`, not a separate `properties` table.** Each entry has `address`, `beds`, `baths`, `sqft`, `notes`, `price`, etc. (often empty strings on imported clients). There is no `public.properties` table Ã¢ÂÂ don't try to JOIN one.
- **Commercial/Janitorial booking field handling in `saveNewAppt` (~line 5330):** beds and baths are intentionally cleared for commercial (`isCommercialAppt?'':...`) since they don't apply, but `apptSqft` is read for ALL service types including commercial Ã¢ÂÂ pricing depends on it. Don't reintroduce a commercial guard on sqft.
...
- Cleaner clicks "Sign in with Google" â Supabase OAuth.
...
- This is admin-email matching, not email-based auth â `cleaners.email` is contact info, NOT used for portal auth.
...
### DB cleanup pending
The Supabase database still contains the now-orphaned objects from the deleted stack. They should be dropped manually:
```sql
...
```
Run this in the Supabase SQL editor when convenient. Nothing in the codebase reads or writes those objects anymore (verified by grep on 2026-04-30).

### Mobile UX

`portal.html` has a mobile CSS pass (commit 169450d): inputs at 16px (no iOS zoom on focus), buttons ≥52px tall and full-width on phones, safe-area insets for notches, stacked `.row` content, and reduced padding on cards. Tested viewport: 480px and below.

### Stale invite-status UI

The CRM previously rendered a "pending invite" status badge per cleaner via `refreshInviteStatus()`, which queried the dropped `cleaner_invites` table. The function was stubbed to a no-op (commit 0da134f) that hides the `#cp-portal-status` element so the UI stays clean. Don't restore the original — there are no tokens to track in the new flow.
...
Supabase project's default mailer is rate-limited. Magic links to the VA (Leo) weren't being delivered. Fix in progress: configure custom SMTP in Supabase project Auth settings using Resend (host: `smtp.resend.com`, port: 465, username: `resend`, password: Resend API key, sender: `noreply@hawaiinaturalclean.com`). All fields filled in the SMTP config form except password Ã¢ÂÂ Dane to paste the Resend API key himself.

### Client profile additions (next slice)
- **Upcoming appointments** Ã¢ÂÂ replace the "Next job: Not scheduled" line with a list of all future scheduled appointments (mirror of Job History but filtered `date >= today`).
- Optional polish: "paid" badge in Job History (logic added but doesn't appear visually Ã¢ÂÂ may need a CSS color tweak).
- Decide whether `cl-mrr` should mean "rolling 30 days" (current) or "current calendar month".

### VA login & security
- Flip `TEST_MODE_DANE_ONLY = true Ã¢ÂÂ false` in 3 places once Dane confirms ready: `run-task-automations.js`, `run-job-completions.js`, `saveApptEdit` in `index.html` (~line 3650).
- Consider a `VA_EMAILS` allowlist (currently only `ADMIN_EMAILS` exists; non-admin users get the `hnc-va-user` class).
- Reporting page is hidden for VA via CSS only Ã¢ÂÂ devtools could reveal it. Consider also hiding Automations + Broadcasts.
- Visible toggle button to open the login overlay.

### Other queued items
- Native Automations Builder UI inside the CRM (visual "When Ã¢ÂÂ If Ã¢ÂÂ Do", with toggleable rules and run logs).
...
*Last updated: April 30, 2026 Ã¢ÂÂ Cleaner Portal frontend shipped: cleaner-portal.html public page + invite button on cleaner profile + status row. Migrations still need a manual run on Supabase.*