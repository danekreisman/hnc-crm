...
export default async function handler(req, res) {
  ...
  try {
          ...
          if (action === 'send_invoice') {
              ...
              try { var _b2=req.body||{}; await fetch(process.env.SUPABASE_URL+'/rest/v1/invoices',{method:'POST',headers:{'apikey':process.env.SUPABASE_SERVICE_ROLE_KEY,'Authorization':'Bearer '+process.env.SUPABASE_SERVICE_ROLE_KEY,'Content-Type':'application/json','Prefer':'return=minimal'},body:JSON.stringify({client_id:_b2.clientId||null,client_name:_b2.clientName||_b2.client_name||null,description:_b2.description||null,amount:finalized.subtotal?finalized.subtotal/100:(_b2.amount||0),tax:finalized.tax?finalized.tax/100:0,total:finalized.total?finalized.total/100:(_b2.amount||0),stripe_invoice_id:finalized.id,payment_terms:_b2.paymentTerms||'NET 30',notes:_b2.notes||null,status:'open'})}); } catch(_e){ console.error('[invoice] db save:',_e.message); }
