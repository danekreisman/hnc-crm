...
*Last updated: May 7, 2026 (evening, 2nd batch) — **Calendar banner for pending booking requests + 3-button Edit/Accept/Deny modal.** Replaces the "I missed the notification, where's the request?" problem with a persistent banner that stays visible until each request is handled. **Banner**: new `<div id="public-booking-banner">` inserted at the top of `<div id="view-calendar">` (before `.cal-header`). Populated by `renderPublicBookingBanner()` from `_tasks.open` filtered to `type='review_public_booking'`. Sorted rush-fee descending, then requested_date ascending — same-day bookings always rise to the top. Each row shows: customer name, rush badge (⚡ SAME-DAY/NEXT-DAY/2-DAY when applicable), new-lead-vs-returning tag, requested date+time, service, and total. Click anywhere on a row → `openPublicBookingReview(taskId)`. Banner auto-hides when no open `review_public_booking` tasks. **Render hook**: `renderTasks()` (in index.html, called by `loadTasks()` and from accept/decline post-actions) now calls `renderPublicBookingBanner()` at the end, so the banner stays in sync without a separate subscription. Realtime tasks subscription doesn't exist yet — banner refreshes when `loadTasks` runs (on app boot, after accept, after decline, after view switch in some paths). If a request comes in while the user is sitting idle on the calendar, they'll see it on the next view switch or the next page load. A future improvement is wiring Supabase Realtime on `tasks` to push-update the banner live, same pattern as the leads-realtime subscription added 2026-05-05. **Three-button modal**: replaced [Accept & schedule] [Cancel] with [Accept] [Edit] [Deny]. Cancel is gone — the X close button in the panel header is the back-out path. Deny is styled red (#b91c1c text + #fca5a5 border) so it doesn't read as a primary action. **Edit mode**: clicking Edit toggles a hidden `<div id="pbr-edit-fields">` panel that exposes editable inputs for service (select), frequency (select), beds, baths, sqft, and total price (post-tax + post-rush). The read-only `pbr-service-block` and `pbr-pricing-block` hide while edit-mode is on; the Edit button label flips to "Done editing." Clicking it again toggles back. State is per-modal-open — `openPublicBookingReview` always resets edit-mode OFF and pre-fills the inputs from `extracted_data`. **Backend overrides**: `acceptPublicBooking` schema (in `validate.js`) extended with optional `service`, `frequency`, `beds`, `baths`, `sqft`, `totalPrice` fields. `accept-public-booking.js` reads each override (falls back to `x.<field>` if not provided) and applies them to BOTH the appointment payload AND the client_data passed to `book_lead_atomic` (so a newly-created client gets consistent values). For `totalPrice`, the server splits it back into pre-tax base and tax using the formula `base = (total - rushFee) / (1 + TAX_RATE)`, then `tax = base * TAX_RATE` — so the appointments table always has `base_price` + `tax` + `total_price` columns filled correctly even when Dane just types one round number into the modal. **Files**: `index.html` (banner DIV in calendar view, `renderPublicBookingBanner` function, `renderTasks` hook, modal save-bar replaced with 3 buttons + edit-fields panel inserted, `togglePublicBookingEditMode` + `confirmPublicBookingDeny` added, `openPublicBookingReview` resets edit state + pre-fills edit inputs, `confirmPublicBookingAccept` reads edit-mode values into payload), `api/utils/validate.js` (5 override fields added to `acceptPublicBooking`), `api/accept-public-booking.js` (override resolution + price recompute from totalPrice). **No migrations**. **Test**: submit any booking via book.html. Open the CRM, navigate to Calendar — should see a blue banner at the top with the request listed, sorted with any rush requests on top. Click the row → modal opens. Click Edit → service/frequency/beds/baths/sqft/total become editable. Change the total to e.g. 200.00, click Done editing, click Accept → an appointment lands in the calendar with `base_price` ≈ 191.05, `tax` ≈ 9.00, `total_price` = 200.00 (assuming no rush fee). The banner row disappears; the task moves to Completed. Click Deny on a different request → banner row disappears, task moves to Completed (Decline path), no appointment created. **Open items**: (1) Realtime banner updates — currently relies on next `loadTasks` to refresh, so a same-day request won't show until view switch or refresh if Dane's idle on calendar. Would be 5 lines to wire: subscribe to `INSERT/UPDATE/DELETE` on the `tasks` table filtered to `type=review_public_booking`, call `renderPublicBookingBanner()` in the handler. (2) Cleaner-assignment dropdown still not in the modal — assigned from calendar after acceptance. (3) Send-confirmation-email checkbox still defaults OFF.*
